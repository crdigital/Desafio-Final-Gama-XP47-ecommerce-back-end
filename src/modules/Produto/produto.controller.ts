...
class ProdutoController {
    ...
    public async create(req: Request, res: Response) {
        try {
            ...
            if( tipo != '1'){
                return res.status(400).json({erro:'Somente usuários administradores podem criar produtos.'});                
            }
    ...
    public async update(req: Request, res: Response) {
        try {
            ...
            if( tipo != '1'){
                return res.status(400).json({erro:'Somente usuários administradores podem atualizar produtos.'});                
            }
    ...
    public async delete(req: Request, res: Response) {
        try {
            ...
            if( tipo != '1'){
                return res.status(400).json({erro:'Somente usuários administradores podem deletar produtos.'});                
            }
    ...
    public async listByCategoria(req: Request, res: Response){
        try {
            
            let { id } = req.params; 

            const prodByCat = await ProdutoModel.findAll({ where: { idCategoria: id } });

            return res.status(200).json(prodByCat);

        } catch (error) {
            return res.status(400).json({erro: "Erro na requisição", error});
        }
    }
}
